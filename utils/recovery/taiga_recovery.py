...
def main(argv: list[str] | None = None):
    ...
    for event in events:  # Iterate over the events to backfill
        ...
        raw = fetch_entities(
            event, pid, start, end
        )  # Get the raw data from the Taiga API for the event
        docs = [
            converter(r, ns.prj) for r in raw
        ]  # Convert the raw data to the MongoDB schema using the converter function
        coll = get_collection(
            f"taiga_{ns.prj}.{event}"
        )  # Get the MongoDB collection for the event
