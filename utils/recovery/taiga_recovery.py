...
def get_project_id_by_username_id(project_name: str, token: str) -> int:
    ...
    r = requests.get(f"{TAIGA_API_URL}/projects", headers=h, params={"member": uid}, timeout=10)
    r.raise_for_status()
    for p in r.json():
...
def fetch_entities(entity: str, project: int, start: Optional[datetime] = None, end: Optional[datetime] = None) -> List[Dict]:
    ...
    r = requests.get(f"{TAIGA_API_URL}/{endpoint_path}", headers=headers, params=params, timeout=30)  # In the request we add the headers and params
    r.raise_for_status()
    return r.json()
