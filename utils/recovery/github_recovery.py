...
def collect_github(
    ...
    for ev in events: # Start iterating over the events to collect
        ...
        if ev == "commits": #First commits
            ...
            for (
                raw
            ) in payloads:  # All the raw payloads, parse them and insert in collection
            ...
            for (
                ...
                raw_issues
            ):  # All the raw payloads, parse them and insert in collection
                payload = {
    ...
    for k in (
        ...
        "issues",
        "pull_requests",
    ):  # Log the counters of each event type
        if k in events:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Back-fill of GITHUB for a project, to insert it in MongoDB"
    )
    ap.add_argument("--org", required=True, help="Organization name on Github")
    ap.add_argument(
        "--repo",
        help="Repository name on Github, if not provided, it will backfill all repositories in the organization",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--events",
        default="commits,issues,pull_requests",
        help="List separated by commas pf the events to backfill, by default: commits,issues,pull_requests",
    )
    ap.add_argument(
        "--from-date",
        help="Date FROM the backfill will be made, must be in format (YYYY-MM-DD), can also put a full date with time (2025-05-01T14:30)",
    )
    ap.add_argument(
        "--to-date",
        help="Date UNTIL the backfill will be made, must be in format (YYYY-MM-DD), can also put a full date with time (2025-05-01T14:30)",
    )
    ap.add_argument(
        "--quality-model",
        default="default",
    ...
    for repo in repos:
        collect_github(
            ...
            since=since,
            until=until,
            quality_model=ns.quality_model,
        )
